...
Event::Event(void) { _esEventStatus = EVSET_EVENTBUILDED; }
...

{
	...
	if (_esEventStatus == EVSET_EVENTINITIALIZED)
	{
		...
		{
			Error err = EventsSetErrors(__FILE__, __LINE__, EVSET_EVENT_FINISH_FAILED);
		}
	}
}

Event::Event(const Event &) { assert(1 == 0); }
...
*/
Error Event::init(
	const char *pSource, const char *pDestination, long lTypeIdentifier, const char *pTypeIdentifier,
...

{
	...
	if (pSource == (const char *)NULL || pDestination == (const char *)NULL || pTypeIdentifier == (const char *)NULL)
	{
	...
	{
		Error err = EventsSetErrors(__FILE__, __LINE__, EVSET_OPERATION_NOTALLOWED, 1, (long)_esEventStatus);
	...
	_ullExpirationLocalDateTimeInMilliSecs = ullExpirationLocalDateTimeInMilliSecs;

	{
		...
		Datetime::nowUTCInMilliSecs(&_ullCreationUTCTimeInMilliSecs, (long *)NULL);

		// the format date is yyyy-mm-dd-hh24-mi-ss-milliss
		sprintf(
			_pCreationLocalDateTime, "%04lu-%02lu-%02lu %02lu:%02lu:%02lu:%04lu", (unsigned long)(tmDateTime.tm_year + 1900),
...
const char *Event::getTypeIdentifier(void) { return (const char *)_bTypeIdentifier; }
...
#else
*/
Error Event::getExpirationLocalDateTimeInMilliSecs(unsigned long long *pullExpirationLocalDateTimeInMilliSecs)
// #endif

{
	...
	*pullExpirationLocalDateTimeInMilliSecs = _ullExpirationLocalDateTimeInMilliSecs;

	return errNoError;
}

#ifdef WIN32
Error Event::setExpirationLocalDateTimeInMilliSecs(__int64 ullExpirationLocalDateTimeInMilliSecs)
#else
Error Event::setExpirationLocalDateTimeInMilliSecs(unsigned long long ullExpirationLocalDateTimeInMilliSecs)
#endif
